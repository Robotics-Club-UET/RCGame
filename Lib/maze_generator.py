...
class Cell:
    ...
    def draw(self, sc):
        ...
        if self.walls['top']:
            pygame.draw.line(sc, pygame.Color('bisque4'), (x, y), (x + TILE, y), self.thickness)
        if self.walls['right']:
            pygame.draw.line(sc, pygame.Color('bisque4'), (x + TILE, y), (x + TILE, y + TILE), self.thickness)
        if self.walls['bottom']:
            pygame.draw.line(sc, pygame.Color('bisque4'), (x + TILE, y + TILE), (x, y + TILE), self.thickness)
        if self.walls['left']:
            pygame.draw.line(sc, pygame.Color('bisque4'), (x, y + TILE), (x, y), self.thickness)

    def draw_current_cell(self, sc):
        pygame.draw.rect(sc, pygame.Color("saddlebrown"), (x + 2, y + 2, TILE - 2, TILE - 2))

    def get_rects(self, sc=None):
        rects = []
        x, y = self.x * TILE, self.y * TILE
        if sc != None:
            pygame.draw.rect(sc, pygame.Color("saddlebrown"), (x + 2, y + 2, TILE - 2, TILE - 2))
    ...
    def players(self, sc, signal_control):
        x, y = self.x * TILE, self.y * TILE
        if signal_control == "down":
