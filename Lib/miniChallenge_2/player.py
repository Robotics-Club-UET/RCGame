...
class Enemy:
    ...
    def draw(self, screen):
        image = pygame.image.load("../Lib/miniChallenge_2/robot_assassin.png").convert_alpha()
        image = pygame.transform.scale(image, (self.radius*2, self.radius*2))
        # img_surface = pygame.Surface((int(self.x),int(self.y)), pygame.SRCALPHA)
        pg.draw.circle(screen, (255,255,0), (int(self.x), int(self.y)), \
            self.radius)
